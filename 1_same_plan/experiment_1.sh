...
mkdir -p runs/p$PROBLEM/global_results
...
for i in `seq 2 $NUMBER_OF_RUNS`;
do
    python $SCRIPT_PATH/is_file_content_same.py $GR_BASE_DIR/1/mercury.plan.1 $GR_BASE_DIR/$i/mercury.plan.1 >> $GR_BASE_DIR/global_results/are_plans_equal_result.txt
done
