...
function run(){
    ...
    mkdir -p runs/d$DOMAIN_COST/$1/local_results
    mv elapsed.time mercury.plan.* runs/d$DOMAIN_COST/$1
...
mkdir -p runs/d$DOMAIN_COST/global_results/are_plans_equal
...
mkdir -p runs/d$DOMAIN_COST/global_results/does_plan_solve_goal
for i in `seq 1 $NUMBER_OF_RUNS`;
do
    command $CURRENT_DIR/../plan_validation/build/plan_validator $DOMAIN_PATH $PROBLEM_PATH $GR_BASE_DIR/1/mercury.plan.$i >> runs/d$DOMAIN_COST/global_results/does_plan_solve_goal/plan_solve_goal.txt
done
