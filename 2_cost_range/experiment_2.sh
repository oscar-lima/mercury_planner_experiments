...
if [[ $# != 1 ]]; then
    echo 'USAGE ./experiment_2.sh domain_number'
    exit 1
fi
...
DOMAIN_PATH=$DOMAIN_BASE_DIR/$DOMAIN/domains/generated_domains/domain$DOMAIN_COST.pddl
...
python $SCRIPT_PATH/is_file_content_same.py $CURRENT_DIR/runs/d$DOMAIN_COST/$3/mercury.plan.$1 $CURRENT_DIR/runs/d$DOMAIN_COST/$3/mercury.plan.$2
}
...
function run(){
    ...
    mkdir -p $CURRENT_DIR/runs/d$DOMAIN_COST/$1/local_results
    mv elapsed.time mercury.plan.* $CURRENT_DIR/runs/d$DOMAIN_COST/$1
    ...
    do
        compare_plans 1 $i $1 >> $CURRENT_DIR/runs/d$DOMAIN_COST/$1/local_results/are_plans_equal_result.txt
    done

    echo '[EXPERIMENTS] 3. generating plan lengths'
    python $SCRIPT_PATH/calc_lenght.py $CURRENT_DIR/runs/d$DOMAIN_COST/$1 > $CURRENT_DIR/runs/d$DOMAIN_COST/$1/local_results/plan_lengths.txt

    echo '[EXPERIMENTS] 4. generating planning times'
    python $SCRIPT_PATH/calc_time.py $CURRENT_DIR/runs/d$DOMAIN_COST/$1 > $CURRENT_DIR/runs/d$DOMAIN_COST/$1/local_results/planning_times.txt
}
...
mkdir -p $CURRENT_DIR/runs/d$DOMAIN_COST/global_results/are_plans_equal

# global results base directory
GR_BASE_DIR=$CURRENT_DIR/runs/d$DOMAIN_COST
...
mkdir -p $CURRENT_DIR/runs/d$DOMAIN_COST/global_results/does_plan_solve_goal
for i in `seq 1 $NUMBER_OF_RUNS`;
do
    command $CURRENT_DIR/../plan_validation/build/plan_validator $DOMAIN_PATH $PROBLEM_PATH $GR_BASE_DIR/1/mercury.plan.$i >> $CURRENT_DIR/runs/d$DOMAIN_COST/global_results/does_plan_solve_goal/plan_solve_goal.txt
done
